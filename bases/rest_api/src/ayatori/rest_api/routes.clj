...
(defn app-handler
  [context-path {:keys [database lra-engine-input-chan]}]
  (ring/ring-handler
    (ring/router
      [["/swagger.json"
        {:get {:no-doc  true
        ...
        [[""
         ...
         ["/:code/close"
          {:put {:summary    "close lra"
                 :parameters {:path [:map [:code uuid?]]}
                 :responses  {200 {:body string?}}
                 :handler    (handler/close-lra-handler database lra-engine-input-chan)}}]
         ["/:code/cancel"
          {:put {:summary    "cancel lra"
                 :parameters {:path [:map [:code uuid?]]}
                 :responses  {200 {:body string?}}
                 :handler    (handler/cancel-lra-handler database lra-engine-input-chan)}}]]]]
